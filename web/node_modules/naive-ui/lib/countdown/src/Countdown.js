...
var Countdown_default = (0, vue.defineComponent)({
	name: "Countdown",
	...
	setup(props) {
		...
		function getDisplayValue(info) {
			...
			switch (precision) {
				case 0: return `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}`;
				default: return `${String(hours).padStart(2, "0")}:${String(minutes).padStart(2, "0")}:${String(seconds).padStart(2, "0")}.${String(Math.floor(milliseconds / (precision === 1 ? 100 : precision === 2 ? 10 : 1))).padStart(precision, "0")}`;
			}
		}
		const frame = () => {
			const { precision } = props;
	...
	render() {
		...
		switch (precision) {
			...
			case 2:
				...
				break;
			case 3: timeInfo = getTimeInfo(distance);
		}
		if (render) return render(timeInfo);
		else return getDisplayValue(timeInfo);
	}
});
//#endregion
