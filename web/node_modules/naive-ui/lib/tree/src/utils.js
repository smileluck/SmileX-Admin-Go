...
let vue = require("vue");
//#region src/tree/src/utils.ts
function useMergedCheckStrategy(props) {
	return (0, vue.computed)(() => props.leafOnly ? "child" : props.checkStrategy);
}
...
function keysWithFilter(nodes, pattern, keyField, childrenField, filter) {
	...
	traverse(nodes, childrenField, (node) => {
		...
		if (filter(pattern, node)) {
			...
			for (let i = path.length - 2; i >= 0; --i) if (!keys.has(path[i][keyField])) keys.add(path[i][keyField]);
			else return;
		}
	}, () => {
		path.pop();
...
if (require__utils_env_is_browser.isBrowser && Image) {
			...
			if (filter(pattern, n)) {
				...
				for (let i = path.length - 2; i >= 0; --i) {
					...
					if (!visitedNonTailKeys.has(key)) {
						...
						if (visitedTailKeys.has(key)) visitedTailKeys.delete(key);
					} else break;
				}
			}
...
function treeGetClickTarget(e) {
	if ((0, seemly.happensIn)(e, "checkbox")) return "checkbox";
	else if ((0, seemly.happensIn)(e, "switcher")) return "switcher";
	else return "node";
}
//#endregion
