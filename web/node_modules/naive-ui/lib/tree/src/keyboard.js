...
let vue = require("vue");
//#region src/tree/src/keyboard.tsx
function useKeyboard({ props, fNodesRef, mergedExpandedKeysRef, mergedSelectedKeysRef, mergedCheckedKeysRef, handleCheck, handleSelect, handleSwitcherClick }) {
	...
	const pendingNodeKeyRef = treeSelectInjection ? treeSelectInjection.pendingNodeKeyRef : (0, vue.ref)(mergedSelectedKeys.length ? mergedSelectedKeys[mergedSelectedKeys.length - 1] : null);
	function handleKeydown(e) {
		...
		if (pendingNodeKey === null) {
			...
			if (e.key === "Enter") {
				...
				enterBehavior = props.overrideDefaultNodeClickBehavior?.({ option: tmNode.rawNode }) || null;
				...
				if (pendingNode.isLeaf || !mergedExpandedKeysRef.value.includes(pendingNodeKey)) {
					...
					if (parentTmNode) pendingNodeKeyRef.value = parentTmNode.key;
				} else handleSwitcherClick(pendingNode);
			} else if (e.key === "ArrowRight") {
				const pendingNode = fNodes[fIndex];
				if (pendingNode.isLeaf) return { enterBehavior: null };
				if (!mergedExpandedKeysRef.value.includes(pendingNodeKey)) handleSwitcherClick(pendingNode);
				else {
					fIndex += 1;
					while (fIndex < fNodes.length) {
						if (!fNodes[fIndex].disabled) {
