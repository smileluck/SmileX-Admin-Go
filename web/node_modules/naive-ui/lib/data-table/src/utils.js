...
function getNumberColWidth(col) {
	if (col.type === "selection") return col.width === void 0 ? 40 : (0, seemly.depx)(col.width);
	if (col.type === "expand") return col.width === void 0 ? 40 : (0, seemly.depx)(col.width);
	if ("children" in col) return void 0;
	if (typeof col.width === "string") return (0, seemly.depx)(col.width);
	return col.width;
}
function getStringColWidth(col) {
	if (col.type === "selection") return require__utils_css_format_length.formatLength(col.width ?? 40);
	if (col.type === "expand") return require__utils_css_format_length.formatLength(col.width ?? 40);
	if ("children" in col) return;
	return require__utils_css_format_length.formatLength(col.width);
}
...
function createCustomWidthStyle(column, resizedWidth) {
	...
	return {
		width,
		minWidth: require__utils_css_format_length.formatLength(minWidth) || width,
		maxWidth: require__utils_css_format_length.formatLength(maxWidth)
	};
}
...
//#endregion
exports.EXPAND_COL_WIDTH = EXPAND_COL_WIDTH;
exports.SELECTION_COL_WIDTH = SELECTION_COL_WIDTH;
