...
function useCheck(props, data) {
	...
	const mergedCheckedRowKeysRef = (0, vue.computed)(() => mergedCheckState.value.checkedKeys);
	const mergedInderminateRowKeysRef = (0, vue.computed)(() => mergedCheckState.value.indeterminateKeys);
	const mergedCheckedRowKeySetRef = (0, vue.computed)(() => {
		return new Set(mergedCheckedRowKeysRef.value);
	});
	const mergedInderminateRowKeySetRef = (0, vue.computed)(() => {
		return new Set(mergedInderminateRowKeysRef.value);
	});
	const countOfCurrentPageCheckedRowsRef = (0, vue.computed)(() => {
	...
	const countOfCurrentPageDisabledRowsRef = (0, vue.computed)(() => {
		return paginatedDataRef.value.filter((item) => item.disabled).length;
	});
	const someRowsCheckedRef = (0, vue.computed)(() => {
	...
	const allRowsCheckedRef = (0, vue.computed)(() => {
		const { length } = paginatedDataRef.value;
	...
	const headerCheckboxDisabledRef = (0, vue.computed)(() => {
	...
	function doUpdateCheckedRowKeys(keys, row, action) {
		...
		if (_onUpdateCheckedRowKeys) require__utils_vue_call.call(_onUpdateCheckedRowKeys, keys, rows, {
			row,
			action
		});
		if (onUpdateCheckedRowKeys) require__utils_vue_call.call(onUpdateCheckedRowKeys, keys, rows, {
			row,
			action
		});
		if (onCheckedRowKeysChange) require__utils_vue_call.call(onCheckedRowKeysChange, keys, rows, {
			row,
			action
		});
	...
	function doCheckAll(checkWholeTable = false) {
		...
		(checkWholeTable ? treeMateRef.value.treeNodes : paginatedDataRef.value).forEach((tmNode) => {
			if (!tmNode.disabled) rowKeysToCheck.push(tmNode.key);
		});
	...
	function doUncheckAll(checkWholeTable = false) {
		...
		(checkWholeTable ? treeMateRef.value.treeNodes : paginatedDataRef.value).forEach((tmNode) => {
			if (!tmNode.disabled) rowKeysToUncheck.push(tmNode.key);
		});
