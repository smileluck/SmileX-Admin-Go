...
function getDefaultSorterFn(columnKey) {
	return (row1, row2) => {
		...
		if (value1 === null || value1 === void 0) {
			...
			return -1;
		} else if (value2 === null || value2 === void 0) return 1;
		else if (typeof value1 === "number" && typeof value2 === "number") return value1 - value2;
...
function useSorter(props, { dataRelatedColsRef, filteredDataRef }) {
	...
	const uncontrolledSortStateRef = (0, vue.ref)(defaultSortState);
	const mergedSortStateRef = (0, vue.computed)(() => {
		...
		if (Array.isArray(uncontrolledSortState)) return uncontrolledSortState;
		else if (uncontrolledSortState) return [uncontrolledSortState];
		else return [];
	});
	const sortedDataRef = (0, vue.computed)(() => {
		...
		if (activeSorters.length) return filteredDataRef.value.slice().sort((tmNode1, tmNode2) => {
			...
			activeSorters.some((sorterState) => {
				...
				if (compareFn && order) {
					...
					if (compareResult !== 0) {
						compareResult = compareResult * require_data_table_src_utils.getFlagOfOrder(order);
						return true;
					}
				}
				return false;
	...
	function doUpdateSorter(sortState) {
		...
		if (_onUpdateSorter) require__utils_vue_call.call(_onUpdateSorter, sortState);
		if (onUpdateSorter) require__utils_vue_call.call(onUpdateSorter, sortState);
		if (onSorterChange) require__utils_vue_call.call(onSorterChange, sortState);
		uncontrolledSortStateRef.value = sortState;
	}
	function sort(columnKey, order = "ascend") {
		if (!columnKey) clearSorter();
		else {
	...
	function updateSortStatesByNewSortState(sortStates, sortState) {
		...
		if (index !== void 0 && index >= 0) sortStates[index] = sortState;
		else sortStates.push(sortState);
	}
	return {
		clearSorter,
		sort,
