...
function useMenuChild(props) {
	const NMenu = (0, vue.inject)(require_menu_src_context.menuInjectionKey);
	const { props: menuProps, mergedCollapsedRef } = NMenu;
	const NSubmenu = (0, vue.inject)(require_menu_src_context.submenuInjectionKey, null);
	const NMenuOptionGroup = (0, vue.inject)(require_menu_src_context.menuItemGroupInjectionKey, null);
	const horizontalRef = (0, vue.computed)(() => {
		return menuProps.mode === "horizontal";
	});
	const dropdownPlacementRef = (0, vue.computed)(() => {
		if (horizontalRef.value) return menuProps.dropdownPlacement;
		if ("tmNodes" in props) return "right-start";
		return "right";
	});
	const maxIconSizeRef = (0, vue.computed)(() => {
		return Math.max(menuProps.collapsedIconSize ?? menuProps.iconSize, menuProps.iconSize);
	});
	return {
		dropdownPlacement: dropdownPlacementRef,
		activeIconSize: (0, vue.computed)(() => {
			if (!horizontalRef.value && props.root && mergedCollapsedRef.value) return menuProps.collapsedIconSize ?? menuProps.iconSize;
			else return menuProps.iconSize;
		}),
		maxIconSize: maxIconSizeRef,
		paddingLeft: (0, vue.computed)(() => {
		...
		iconMarginRight: (0, vue.computed)(() => {
			const { collapsedWidth, indent, rootIndent } = menuProps;
