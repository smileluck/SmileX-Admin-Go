...
function filterOptions(originalOpts, filter, pattern, childrenField) {
	...
	function traverse(options) {
		...
		for (const option of options) if (getIsGroup(option)) {
			...
			if (children.length) filteredOptions.push(Object.assign({}, option, { [childrenField]: children }));
		} else if (getIgnored(option)) continue;
		else if (filter(pattern, option)) filteredOptions.push(option);
