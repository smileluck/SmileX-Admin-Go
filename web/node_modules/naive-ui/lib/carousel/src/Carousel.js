...
const require_carousel_styles_light = require("../styles/light.js");
const require_carousel_src_CarouselContext = require("./CarouselContext.js");
const require_carousel_src_CarouselArrow = require("./CarouselArrow.js");
const require_carousel_src_CarouselDots = require("./CarouselDots.js");
const require_carousel_src_CarouselItem = require("./CarouselItem.js");
const require_carousel_src_styles_index_cssr = require("./styles/index.cssr.js");
const require_carousel_src_utils_duplicatedLogic = require("./utils/duplicatedLogic.js");
const require_carousel_src_utils_event = require("./utils/event.js");
const require_carousel_src_utils_index = require("./utils/index.js");
...
var Carousel_default = (0, vue.defineComponent)({
	name: "Carousel",
	props: carouselProps,
	slots: Object,
	setup(props) {
		...
		const slidesElRef = (0, vue.ref)(null);
		const slideElsRef = (0, vue.ref)([]);
		const slideVNodesRef = { value: [] };
		const verticalRef = (0, vue.computed)(() => props.direction === "vertical");
		const sizeAxisRef = (0, vue.computed)(() => verticalRef.value ? "height" : "width");
		const spaceAxisRef = (0, vue.computed)(() => verticalRef.value ? "bottom" : "right");
		const sequenceLayoutRef = (0, vue.computed)(() => props.effect === "slide");
		const duplicatedableRef = (0, vue.computed)(() => props.loop && props.slidesPerView === 1 && sequenceLayoutRef.value);
		const userWantsControlRef = (0, vue.computed)(() => props.effect === "custom");
		const displaySlidesPerViewRef = (0, vue.computed)(() => !sequenceLayoutRef.value || props.centeredSlides ? 1 : props.slidesPerView);
		const realSlidesPerViewRef = (0, vue.computed)(() => userWantsControlRef.value ? 1 : props.slidesPerView);
		const autoSlideSizeRef = (0, vue.computed)(() => displaySlidesPerViewRef.value === "auto" || props.slidesPerView === "auto" && props.centeredSlides);
		const perViewSizeRef = (0, vue.ref)({
			width: 0,
			height: 0
		});
		const slideSizesTrigger = (0, vue.ref)(0);
		const slideSizesRef = (0, vue.computed)(() => {
			...
			if (autoSlideSize) return slidesEls.map((slide) => require_carousel_src_utils_index.calculateSize(slide));
		...
		const slideTranlatesRef = (0, vue.computed)(() => {
		...
		const isMountedRef = (0, vue.ref)(false);
		const transitionStyleRef = (0, vue.computed)(() => {
			const { transitionStyle } = props;
			return transitionStyle ? require__utils_vue_keep.keep(transitionStyle, transitionProperties) : {};
		});
		const speedRef = (0, vue.computed)(() => userWantsControlRef.value ? 0 : require_carousel_src_utils_index.resolveSpeed(transitionStyleRef.value.transitionDuration));
		const slideStylesRef = (0, vue.computed)(() => {
			const { value: slidesEls } = slideElsRef;
			if (!slidesEls.length) return [];
			...
			const getSlideSize = (index) => {
				if (useComputedSize) {
					const { value: axis } = sizeAxisRef;
					return { [axis]: `${slideSizesRef.value[index][axis]}px` };
				}
			};
		...
		const totalViewRef = (0, vue.computed)(() => {
			const { value: slidesPerView } = displaySlidesPerViewRef;
			const { length: totalSlides } = slideElsRef.value;
			if (slidesPerView !== "auto") return Math.max(totalSlides - slidesPerView, 0) + 1;
			else {
				const { value: slideSizes } = slideSizesRef;
				...
				return require_carousel_src_utils_index.clampValue(i + 1, 1, length);
			}
		});
		const displayTotalViewRef = (0, vue.computed)(() => require_carousel_src_utils_duplicatedLogic.getDisplayTotalView(totalViewRef.value, duplicatedableRef.value));
		const defaultRealIndex = require_carousel_src_utils_duplicatedLogic.getRealIndex(props.defaultIndex, duplicatedableRef.value);
		const uncontrolledDisplayIndexRef = (0, vue.ref)(require_carousel_src_utils_duplicatedLogic.getDisplayIndex(defaultRealIndex, totalViewRef.value, duplicatedableRef.value));
		const mergedDisplayIndexRef = (0, vooks.useMergedState)((0, vue.toRef)(props, "currentIndex"), uncontrolledDisplayIndexRef);
		const realIndexRef = (0, vue.computed)(() => require_carousel_src_utils_duplicatedLogic.getRealIndex(mergedDisplayIndexRef.value, duplicatedableRef.value));
		function toRealIndex(index) {
			index = require_carousel_src_utils_index.clampValue(index, 0, totalViewRef.value - 1);
			const displayIndex = require_carousel_src_utils_duplicatedLogic.getDisplayIndex(index, totalViewRef.value, duplicatedableRef.value);
		...
		function getRealPrevIndex(index = realIndexRef.value) {
			return require_carousel_src_utils_duplicatedLogic.getPrevIndex(index, totalViewRef.value, props.loop);
		}
		function getRealNextIndex(index = realIndexRef.value) {
			return require_carousel_src_utils_duplicatedLogic.getNextIndex(index, totalViewRef.value, props.loop);
		}
		...
		function to(index) {
			const realIndex = require_carousel_src_utils_index.clampValue(require_carousel_src_utils_duplicatedLogic.getRealIndex(index, duplicatedableRef.value), 0, totalViewRef.value);
		...
		let previousTranslate = 0;
		const translateStyleRef = (0, vue.ref)({});
		...
		function fixTranslate(speed = 0) {
			if (sequenceLayoutRef.value) translateTo(realIndexRef.value, speed);
		...
		function getTranslate(index) {
			let translate;
			if (index >= totalViewRef.value - 1) translate = getLastViewTranslate();
			else translate = slideTranlatesRef.value[index] || 0;
			return translate;
		}
		function getLastViewTranslate() {
			if (displaySlidesPerViewRef.value === "auto") {
				...
				let overallSize;
				if (lastTranslate === void 0) overallSize = perViewSize;
				else {
					const { value: slideSizes } = slideSizesRef;
		...
		require_carousel_src_CarouselContext.provideCarouselContext(carouselContext);
		...
		function getSlideStyle(slide) {
			...
			if (index !== -1) {
				...
				return (0, vue.normalizeStyle)(styles);
			}
		}
		let dragStartX = 0;
		let dragStartY = 0;
		...
		function handleTouchstart(event) {
			...
			if (!slidesElRef.value?.contains((0, seemly.getPreciseEventTarget)(event))) return;
			globalDragging = true;
			dragging = true;
			...
			const touchEvent = require_carousel_src_utils_event.isTouchEvent(event) ? event.touches[0] : event;
			if (verticalRef.value) dragStartY = touchEvent.clientY;
			else dragStartX = touchEvent.clientX;
			if (props.touchable) {
				(0, evtd.on)("touchmove", document, handleTouchmove);
				(0, evtd.on)("touchend", document, handleTouchend);
				(0, evtd.on)("touchcancel", document, handleTouchend);
			}
			if (props.draggable) {
				(0, evtd.on)("mousemove", document, handleTouchmove);
				(0, evtd.on)("mouseup", document, handleTouchend);
			}
		}
		function handleTouchmove(event) {
			...
			const perViewSize = perViewSizeRef.value[axis];
			dragOffset = require_carousel_src_utils_index.clampValue(offset, -perViewSize, perViewSize);
		...
		function handleTouchend() {
			...
			if (currentIndex === realIndex) {
				...
				const { value: axis } = sizeAxisRef;
				const perViewSize = perViewSizeRef.value[axis];
				if (dragOffset > perViewSize / 2 || dragOffset / timeElapsed > .4) prev();
				else if (dragOffset < -perViewSize / 2 || dragOffset / timeElapsed < -.4) next();
			}
		...
		function resetDragStatus() {
			...
			dragOffset = 0;
			dragStartTime = 0;
			(0, evtd.off)("touchmove", document, handleTouchmove);
			(0, evtd.off)("touchend", document, handleTouchend);
			(0, evtd.off)("touchcancel", document, handleTouchend);
			(0, evtd.off)("mousemove", document, handleTouchmove);
			(0, evtd.off)("mouseup", document, handleTouchend);
		}
		...
		function handleMousewheel(event) {
			...
			let rx = 0;
			let ry = 0;
			if (verticalRef.value) ry = m;
			else rx = m;
			const responseStep = 10;
			if (ry * deltaY >= responseStep || rx * deltaX >= responseStep) {
				if (m === nextMultiplier && !isNextDisabled()) next();
				else if (m === prevMultiplier && !isPrevDisabled()) prev();
			}
		}
		function handleResize() {
			perViewSizeRef.value = require_carousel_src_utils_index.calculateSize(selfElRef.value, true);
			resetAutoplay();
		}
		...
		(0, vue.onMounted)(() => {
			(0, vue.watchEffect)(resetAutoplay);
		...
		(0, vue.watch)(realIndexRef, (nextRealIndex, lastRealIndex) => {
			...
			if (sequenceLayoutRef.value) {
				if (duplicatedableRef.value) {
					...
					if (expectedTransitionDirection === -1 && lastRealIndex === 1 && nextRealIndex === length - 2) nextRealIndex = 0;
					else if (expectedTransitionDirection === 1 && lastRealIndex === length - 2 && nextRealIndex === 1) nextRealIndex = length - 1;
				}
				translateTo(nextRealIndex, speedRef.value);
			} else fixTranslate();
			expectedTransitionDirection = 0;
		}, { immediate: true });
		(0, vue.watch)([duplicatedableRef, displaySlidesPerViewRef], () => void (0, vue.nextTick)(() => {
			toRealIndex(realIndexRef.value);
		}));
		(0, vue.watch)(slideTranlatesRef, () => {
			if (sequenceLayoutRef.value) fixTranslate();
		}, { deep: true });
		(0, vue.watch)(sequenceLayoutRef, (value) => {
			if (!value) {
				inTransition = false;
				updateTranslate(previousTranslate = 0);
			} else fixTranslate();
		});
		const slidesControlListenersRef = (0, vue.computed)(() => {
			return {
		...
		const arrowSlotPropsRef = (0, vue.computed)(() => ({
			...require__utils_vue_keep.keep(carouselContext, [
				"to",
				"prev",
				"next",
				"isPrevDisabled",
		...
		const dotSlotPropsRef = (0, vue.computed)(() => ({
			total: displayTotalViewRef.value,
			currentIndex: mergedDisplayIndexRef.value,
		...
		const themeRef = require__mixins_use_theme.default("Carousel", "-carousel", require_carousel_src_styles_index_cssr, require_carousel_styles_light.default, props, mergedClsPrefixRef);
		...
		const themeClassHandle = inlineThemeDisabled ? require__mixins_use_css_vars_class.useThemeClass("carousel", void 0, cssVarsRef, props) : void 0;
		return {
	...
	render() {
		...
		const children = defaultSlot && require__utils_vue_flatten.flatten(defaultSlot()) || [];
		let slides = filterCarouselItem(children);
		if (!slides.length) slides = children.map((ch) => ((0, vue.openBlock)(), (0, vue.createBlock)(require_carousel_src_CarouselItem.default, null, { default: () => (0, vue.cloneVNode)(ch) }, 1024)));
		if (this.duplicatedable) slides = require_carousel_src_utils_duplicatedLogic.addDuplicateSlides(slides);
		this.slideVNodes.value = slides;
		if (this.autoSlideSize) slides = slides.map((slide) => ((0, vue.openBlock)(), (0, vue.createBlock)(vueuc.VResizeObserver, { onResize: this.handleSlideResize }, { default: () => slide }, 1032, ["onResize"])));
		this.onRender?.();
		return (0, vue.openBlock)(), (0, vue.createElementBlock)("div", (0, vue.mergeProps)({
			...
			((0, vue.openBlock)(), (0, vue.createBlock)(vueuc.VResizeObserver, { onResize: this.handleResize }, { default: () => ((0, vue.openBlock)(), (0, vue.createElementBlock)("div", {
				ref: "slidesElRef",
				class: require_vdom.normalizeClass(`${mergedClsPrefix}-carousel__slides`),
				role: "listbox",
				style: (0, vue.normalizeStyle)(this.translateStyle),
				onTransitionend: this.handleTransitionEnd
			}, [userWantsControl ? ((0, vue.openBlock)(), (0, vue.createElementBlock)(vue.Fragment, { key: 0 }, [require_vdom.normalizeVNode(() => slides.map((slide, i) => ((0, vue.openBlock)(), (0, vue.createElementBlock)("div", {
				style: (0, vue.normalizeStyle)(slideStyles[i]),
				key: i
			}, [require_vdom.normalizeVNode(() => (0, vue.withDirectives)(((0, vue.openBlock)(), (0, vue.createBlock)(vue.Transition, (0, vue.normalizeProps)((0, vue.guardReactiveProps)(transitionProps)), { default: () => slide }, 1040)), [[vue.vShow, this.isActive(i)]]))], 4))))], 64)) : ((0, vue.openBlock)(), (0, vue.createElementBlock)(vue.Fragment, { key: 1 }, [require_vdom.normalizeVNode(() => slides)], 64))], 46, _hoisted_1)) }, 1032, ["onResize"])),
			require_vdom.normalizeVNode(() => this.showDots && dotSlotProps.total > 1 && require__utils_vue_resolve_slot.resolveSlotWithTypedProps(dotsSlot, dotSlotProps, () => [((0, vue.openBlock)(), (0, vue.createBlock)(require_carousel_src_CarouselDots, {
			...
			require_vdom.normalizeVNode(() => showArrow && require__utils_vue_resolve_slot.resolveSlotWithTypedProps(arrowSlot, arrowSlotProps, () => [((0, vue.openBlock)(), (0, vue.createBlock)(require_carousel_src_CarouselArrow))]))
		], 16, _hoisted_2);
	}
});
function filterCarouselItem(vnodes) {
	return vnodes.reduce((carouselItems, vnode) => {
		if (require_carousel_src_CarouselItem.isCarouselItem(vnode)) carouselItems.push(vnode);
