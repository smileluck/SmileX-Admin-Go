import { throwError, warn } from "../../_utils/naive/warn.mjs";
...
import uploadLight from "../styles/light.mjs";
import { uploadInjectionKey } from "./interface.mjs";
import index_cssr_default from "./styles/index.cssr.mjs";
import "./UploadDragger.mjs";
...
import UploadFileList_default from "./UploadFileList.mjs";
import { createId } from "seemly";
import { Fragment, Teleport, computed, createBlock, createElementBlock, defineComponent, mergeProps, nextTick, normalizeStyle, openBlock, provide, ref, toRef } from "vue";
import { useMergedState } from "vooks";
//#region src/upload/src/Upload.tsx
const _hoisted_1 = ["accept", "multiple", "onChange", "webkitdirectory", "directory"];
/**
...
function createXhrHandlers(inst, file, xhr) {
  ...
  function handleXHRError(e) {
    ...
    fileAfterChange = createSettledFileInfo(inst.onError?.({
  ...
  function handleXHRLoad(e) {
    ...
    xhrMap.delete(file.id);
    fileAfterChange = createSettledFileInfo(inst.onFinish?.({
      file: fileAfterChange,
      event: e
    }) || fileAfterChange);
    doChange(fileAfterChange, e);
...
var Upload_default = defineComponent({
  ...
  props: uploadProps,
  setup(props) {
    if (props.abstract && props.listType === "image-card") throwError("upload", "when the list-type is image-card, abstract is not supported.");
    const {
      mergedClsPrefixRef,
      inlineThemeDisabled,
      mergedRtlRef
    } = useConfig(props);
    const themeRef = useTheme("Upload", "-upload", index_cssr_default, uploadLight, props, mergedClsPrefixRef);
    ...
    const doChange = (fileAfterChange, event, options = {
      ...
      if (append || remove || ~fileIndex) {
        if (append) fileListAfterChange.push(fileAfterChange);else if (remove) fileListAfterChange.splice(fileIndex, 1);else fileListAfterChange.splice(fileIndex, 1, fileAfterChange);
        const {
          onChange
        } = props;
      ...
      } else if (process.env.NODE_ENV !== "production") warn("upload", "File has no corresponding id in current file list.");
    };
    ...
    function handleFileAddition(fileAndEntries, e) {
      ...
      fileAndEntries = fileAndEntries.filter(({
        ...
        if (source === "dnd" && accept?.trim()) return matchType(file.name, file.type, accept);else return true;
      });
      if (max) fileAndEntries = fileAndEntries.slice(0, max - mergedFileListRef.value.length);
      const batchId = createId();
      Promise.all(fileAndEntries.map(async ({
        file,
        entry
      }) => {
        const fileInfo = {
        ...
        fileInfos.forEach(fileInfo => {
          nextTickChain = nextTickChain.then(nextTick).then(() => {
            if (fileInfo) doChange(fileInfo, e, {
              append: true
            });
          });
        });
    ...
    function getFileThumbnailUrlResolver(file) {
      ...
      if (file.url) return file.url;else if (file.file) return createImageDataUrl(file.file);
      return "";
    }
  ...
  render() {
    ...
    const inputNode = (openBlock(), createElementBlock("input", mergeProps(this.inputProps, {
      ref: "inputElRef",
      type: "file",
    ...
    if (this.abstract) return openBlock(), createElementBlock(Fragment, {
      key: 1
    }, [normalizeVNode(() => $slots.default?.()), (openBlock(), createBlock(Teleport, {
      to: "body"
    }, normalizeSlots(inputNode), 1024))], 64);
    onRender?.();
    return openBlock(), createElementBlock("div", {
      class: normalizeClass$1([`${mergedClsPrefix}-upload`, this.rtlEnabled && `${mergedClsPrefix}-upload--rtl`, draggerInsideRef.value && `${mergedClsPrefix}-upload--dragger-inside`, this.dragOver && `${mergedClsPrefix}-upload--drag-over`, this.themeClass]),
      style: normalizeStyle(this.cssVars)
    }, [normalizeVNode(() => inputNode), normalizeVNode(() => this.showTrigger && this.listType !== "image-card" && (openBlock(), createBlock(UploadTrigger_default, null, normalizeSlots($slots), 1024))), normalizeVNode(() => this.showFileList && (openBlock(), createBlock(UploadFileList_default, null, normalizeSlots($slots), 1024)))], 6);
  }
});
//#endregion
export { Upload_default as default, uploadProps };