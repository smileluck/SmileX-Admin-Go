...
import codeLight from "../styles/light.mjs";
import index_cssr_default from "./styles/index.cssr.mjs";
import { computed, createElementBlock, defineComponent, normalizeStyle, onMounted, openBlock, ref, toRef, watch } from "vue";
...
var Code_default = defineComponent({
  ...
  setup(props, {
    ...
    const setCode = () => {
      ...
      if (language) {
        ...
        if (html !== null) {
          if (props.inline) codeEl.innerHTML = html;else {
      ...
      if (maybePreEl) maybePreEl.textContent = code;else {
    ...
    const themeRef = useTheme("Code", "-code", index_cssr_default, codeLight, props, mergedClsPrefixRef);
  ...
  render() {
    ...
    return openBlock(), createElementBlock("code", {
      class: normalizeClass$1([`${mergedClsPrefix}-code`, this.themeClass, wordWrap && `${mergedClsPrefix}-code--word-wrap`, mergedShowLineNumbers && `${mergedClsPrefix}-code--show-line-numbers`]),
      style: normalizeStyle(this.cssVars),
      ref: "codeRef"
    }, [mergedShowLineNumbers ? (openBlock(), createElementBlock("pre", {
      key: 0,
      class: normalizeClass$1(`${mergedClsPrefix}-code__line-numbers`)
    }, [normalizeVNode(() => this.lineNumbers)], 2)) : normalizeVNode(() => null), normalizeVNode(() => this.$slots.default?.())], 6);
  }
});
//#endregion
export { codeProps, Code_default as default };