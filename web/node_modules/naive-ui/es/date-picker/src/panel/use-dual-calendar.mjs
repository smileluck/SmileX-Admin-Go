...
function useDualCalendar(props, type) {
  ...
  const endCalendarDateTimeRef = ref(props.defaultCalendarEndTime ?? (Array.isArray(value) && typeof value[1] === "number" ? value[1] : getTime(addMonths(defaultCalendarStartTime, 1))));
  ...
  function handleCalendarChange(value, oldValue) {
    if (type === "daterange" || type === "datetimerange") {
      if (getYear(value) !== getYear(oldValue) || getMonth(value) !== getMonth(oldValue)) panelCommon.disableTransitionOneTick();
    }
  }
  ...
  function adjustCalendarTimes(byStartCalendarTime) {
    ...
    if (props.bindCalendarMonths || startTime >= endTime) {
      if (byStartCalendarTime) endCalendarDateTimeRef.value = getTime(addMonths(startTime, 1));else startCalendarDateTimeRef.value = getTime(addMonths(endTime, -1));
    }
  }
  ...
  function mergedIsDateDisabled(ts) {
    ...
    if (selectingPhaseRef.value === "start") return isDateDisabled(ts, "start", null);else {
      const {
        value: memorizedStartDateTime
      } = memorizedStartDateTimeRef;
      if (ts < memorizedStartDateTimeRef.value) return isDateDisabled(ts, "start", [memorizedStartDateTime, memorizedStartDateTime]);else return isDateDisabled(ts, "end", [memorizedStartDateTime, memorizedStartDateTime]);
    }
  }
  function syncCalendarTimeWithValue(value) {
    ...
    if (startOfMonth(endMoment) <= startOfMonth(startMoment)) endCalendarDateTimeRef.value = getTime(startOfMonth(addMonths(startMoment, 1)));else endCalendarDateTimeRef.value = getTime(startOfMonth(endMoment));
  }
  function handleDateClick(dateItem) {
    if (!isSelectingRef.value) {
      ...
      if (props.panel && Array.isArray(value)) changeStartEndTime(value[0], value[1], "done");else if (closeOnSelectRef.value && type === "daterange") {
        if (updateValueOnCloseRef.value) closeCalendar();else handleConfirmClick();
      }
    }
  }
  function handleDateMouseEnter(dateItem) {
    if (isSelectingRef.value) {
      ...
      if (dateItem.ts >= memorizedStartDateTimeRef.value) changeStartEndTime(memorizedStartDateTimeRef.value, dateItem.ts, "wipPreview");else changeStartEndTime(dateItem.ts, memorizedStartDateTimeRef.value, "wipPreview");
    }
  }
  ...
  function changeStartDateTime(time) {
    if (typeof time !== "number") time = getTime(time);
    if (props.value === null) panelCommon.doUpdateValue([time, time], props.panel);else if (Array.isArray(props.value)) panelCommon.doUpdateValue([time, Math.max(props.value[1], time)], props.panel);
  }
  function changeEndDateTime(time) {
    if (typeof time !== "number") time = getTime(time);
    if (props.value === null) panelCommon.doUpdateValue([time, time], props.panel);else if (Array.isArray(props.value)) panelCommon.doUpdateValue([Math.min(props.value[0], time), time], props.panel);
  }
  function changeStartEndTime(startTime, endTime, source) {
    if (typeof startTime !== "number") startTime = getTime(startTime);
    if (source !== "shortcutPreview" && source !== "shortcutDone") {
      ...
      if (startDefaultTime) startTime = getTime(set(startTime, startDefaultTime));
      if (endDefaultTime) endTime = getTime(set(endTime, endDefaultTime));
    }
    panelCommon.doUpdateValue([startTime, endTime], props.panel && (source === "done" || source === "shortcutDone"));
  }
  function sanitizeValue(datetime) {
    if (type === "datetimerange") return getTime(startOfSecond(datetime));else if (type === "monthrange") return getTime(startOfMonth(datetime));else return getTime(startOfDay(datetime));
  }
  ...
  function virtualListContainer(type) {
    if (type === "start") return startYearVlRef.value?.listElRef || null;else return endYearVlRef.value?.listElRef || null;
  }
  function virtualListContent(type) {
    if (type === "start") return startYearVlRef.value?.itemsElRef || null;else return endYearVlRef.value?.itemsElRef || null;
  }
  return {
    startDatesElRef,
    endDatesElRef,
