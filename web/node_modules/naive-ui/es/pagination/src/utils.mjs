//#region src/pagination/src/utils.ts
...
function createPageItemsInfo(currentPage, pageCount, pageSlot, showQuickJumpDropdown) {
  ...
  middleStart = Math.max(Math.min(middleStart, lastPage - pageSlot + 3), 3);
  let leftSplit = false;
  let rightSplit = false;
  if (middleStart > 3) leftSplit = true;
  ...
  if (leftSplit) {
    ...
    items.push({
      ...
      active: false,
      label: void 0,
      options: showQuickJumpDropdown ? createRange(2, middleStart - 1) : null
    });
  } else if (lastPage >= 2) items.push({
    type: "page",
    label: 2,
    mayBeFastBackward: true,
    mayBeFastForward: false,
  ...
  for (let i = middleStart; i <= middleEnd; ++i) items.push({
    type: "page",
    label: i,
    mayBeFastBackward: false,
    mayBeFastForward: false,
  ...
  } else if (middleEnd === lastPage - 2 && items[items.length - 1].label !== lastPage - 1) items.push({
    type: "page",
  ...
  if (items[items.length - 1].label !== lastPage) items.push({
    type: "page",
    mayBeFastForward: false,
    mayBeFastBackward: false,
    label: lastPage,
...
function createRange(from, to) {
  const range = [];
  for (let i = from; i <= to; ++i) range.push({
    label: `${i}`,
    value: i
  });
  return range;
...
//#endregion
export { createPageItemsInfo, getDefaultPageSize };