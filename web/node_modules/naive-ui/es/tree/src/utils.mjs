...
function keysWithFilter(nodes, pattern, keyField, childrenField, filter) {
  ...
  traverse(nodes, childrenField, node => {
    ...
    if (filter(pattern, node)) {
      ...
      for (let i = path.length - 2; i >= 0; --i) if (!keys.has(path[i][keyField])) keys.add(path[i][keyField]);else return;
    }
  }, () => {
    path.pop();
  });
...
function filterTree(tree, filter, pattern, keyField, childrenField) {
  ...
  function build(t, sibs) {
    t.forEach(n => {
      ...
      const children = n[childrenField];
      if (children) {
        if (isVisitedTail) sibs.push(n);else {
...
function treeGetClickTarget(e) {
  if (happensIn(e, "checkbox")) return "checkbox";else if (happensIn(e, "switcher")) return "switcher";else return "node";
}
//#endregion
