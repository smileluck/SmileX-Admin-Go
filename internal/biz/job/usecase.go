...
const (
	...
	HandlerMerchantLogCleanup = "merchant_log_cleanup"
...
type MerchantLogCleaner interface {
	CleanupExpired(ctx context.Context) error
}
...
func NewUsecase(repo Repo, lc LogCleaner, ec ExportCleaner, mc MerchantLogCleaner, uc UsageCleaner, nc NotifyCleaner) *Usecase {
	handlers := map[string]Handler{
		HandlerLogCleanup: {
			Key: HandlerLogCleanup, Description: "清理保留期外的登录/操作日志",
			Run: func(ctx context.Context, _ string) (string, error) {
				if err := lc.CleanupExpired(ctx); err != nil {
					return "", err
				}
				return "登录/操作日志清理完成", nil
			},
		},
		...
		HandlerMerchantLogCleanup: {
			Key: HandlerMerchantLogCleanup, Description: "清理保留期外的商户 API 调用日志",
			Run: func(ctx context.Context, _ string) (string, error) {
				if err := mc.CleanupExpired(ctx); err != nil {
					return "", err
				}
				return "商户调用日志清理完成", nil
			},
		},
		HandlerUsageCleanup: {
...
func (uc *Usecase) EnsureSeeded(ctx context.Context) error {
	...
	seeds := []Job{
		{Name: "日志保留期清理", Cron: "0 4 * * *", HandlerKey: HandlerLogCleanup, Status: StatusEnabled,
			Remark: "清理保留期外的登录/操作日志（保留期见 log.retentionDays）"},
		{Name: "导出保留期清理", Cron: "10 4 * * *", HandlerKey: HandlerExportCleanup, Status: StatusEnabled,
			Remark: "清理保留期外的导出记录与产物文件"},
		{Name: "商户日志保留期清理", Cron: "20 4 * * *", HandlerKey: HandlerMerchantLogCleanup, Status: StatusEnabled,
			Remark: "清理保留期外的商户 API 调用日志"},
		{Name: "用量流水保留期清理", Cron: "30 4 * * *", HandlerKey: HandlerUsageCleanup, Status: StatusEnabled,
