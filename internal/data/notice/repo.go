...
// deliveredCondition 送达条件：全体广播 OR 命中定向目标（角色经 user_roles 展开、用户直接命中）
func deliveredCondition(userID uint) (string, []interface{}) {
	return "(notices.scope = ? OR notices.id IN (SELECT notice_id FROM notice_targets WHERE " +
			"(target_type = ? AND target_id IN (SELECT role_id FROM user_roles WHERE user_id = ?)) " +
...
func (r *repo) userNames(ctx context.Context, ids []uint) map[uint]string {
	...
	var rows []model.UserPO
	if err := r.data.DB.WithContext(ctx).Select("id, username, nickname").Where("id IN ?", ids).Find(&rows).Error; err != nil {
		return out
	}
	for _, row := range rows {
		if row.Nickname == "" {
...
// ListUserOptions 送达范围用户选项（启用用户，按用户名/昵称模糊搜索）
func (r *repo) ListUserOptions(ctx context.Context, kw string, limit int) ([]notice.UserOption, error) {
	tx := r.data.DB.WithContext(ctx).Model(&model.UserPO{}).
		Select("id, username, nickname").Where("status = ?", int(bizuser.StatusEnabled))
	if kw != "" {
