...
func newTestRepo(t *testing.T) notice.Repo {
	...
	if err := db.AutoMigrate(
		&model.UserPO{}, &model.RolePO{}, &model.UserRolePO{},
...
func TestTargetedDelivery(t *testing.T) {
	...
	// 用户：alice(id=1,ops 角色) / bob(id=2) / carol(id=3)
	if err := db.Create(&[]model.UserPO{
		{ID: 1, Username: "alice", Nickname: "爱丽丝", Status: 1},
		{ID: 2, Username: "bob", Nickname: "鲍勃", Status: 1},
		{ID: 3, Username: "carol", Nickname: "卡罗尔", Status: 1},
	}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if err := db.Create(&model.UserRolePO{UserID: 1, RoleID: 10}).Error; err != nil {
		t.Fatal(err)
	}
...
// TestUserOptions 用户选项仅含启用用户并支持模糊搜索
func TestUserOptions(t *testing.T) {
	r := newTestRepo(t)
	db := r.(*repo).data.DB
	if err := db.Create(&[]model.UserPO{
		{ID: 1, Username: "alice", Nickname: "爱丽丝", Status: 1},
		{ID: 2, Username: "bob_disabled", Nickname: "停用者", Status: 0},
	}).Error; err != nil {
		t.Fatal(err)
	}
	...
	if len(opts) != 1 || opts[0].Username != "alice" {
		t.Fatalf("options should only include enabled users, got %+v", opts)
	}
